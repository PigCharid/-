...
contract AIDoge is ERC20, Ownable {
    ...
    // 分红ARB的地址
    IERC20 public backToken;     //0x912ce59144191c1204e64559fe8253a0e49e6548 
    ...
    function _dogTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // 如果没有权限 需要已经Launch了  launch之前只有管理员和交易对合约可以转账
        if (!canAddLiquidityBeforeLaunch[sender]) {
            // 有一个Launch的动作 查到已经Launch了
            require(launched(), "Trading not open yet");
        }
        // 查看已经是否免费
        // sender不是免费 并且 接受者不是免费 并且 launch了  需要收手续费  
    ...
    // 
    // 不在交易中 并且 可以 swap 并且 launch了 并且 合约里面有aidog 并且调用地址不是pair
    // 
