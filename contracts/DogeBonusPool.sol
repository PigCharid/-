...
// File: contracts/pool/DogeBonusPool.sol
...
contract DogeBonusPool is Ownable {
    ...
    function add(uint256 _allocPoint, address _token, bool _withUpdate) public onlyOwner {
        require(_token != address(0), "DogeBonusPool: _token is the zero address");

        require(!EnumerableSet.contains(_pairs, _token), "DogeBonusPool: _token is already added to the pool");
    ...
    function setStandardRewardInterval(uint256 interval) public onlyOwner {
        require(interval > 0, "DogeBonusPool: interval must be greater than 0");
        standardRewardInterval = interval;
    }

    function pendingReward(uint256 _pid, address _user) public view returns (uint256) {
        require(_pid <= poolInfo.length - 1, "DogeBonusPool: Can not find this pool");
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require(_amount.add(user.amount) <= maxStakingPerUser, 'DogeBonusPool: exceed max stake');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "DogeBonusPool: Withdraw with insufficient balance");
    ...
    function emergencyRewardWithdraw(uint256 _amount) public onlyOwner {
        require(_amount <= rewardToken.balanceOf(address(this)), 'DogeBonusPool: not enough token');
        rewardToken.safeTransfer(address(msg.sender), _amount);
    }

    function getPoolView(uint256 pid) public view returns (PoolView memory) {
        require(pid < poolInfo.length, "DogeBonusPool: pid out of range");
    ...
    function addCaller(address val) public onlyOwner() {
        require(val != address(0), "DogeBonusPool: val is the zero address");
        _callers.add(val);
    }

    function delCaller(address caller) public onlyOwner returns (bool) {
        require(caller != address(0), "DogeBonusPool: caller is the zero address");
